...
- Em `DataTable`, o estado de ordenação de uma coluna hoje só é comunicado visualmente (o ícone
  de chevron trocando de direção) — não há `aria-sort` no cabeçalho ainda. Se for expor essa
  tela para um público amplo, adicionar `aria-sort="ascending" | "descending" | "none"` ao `<th>`
  correspondente é a melhoria mais direta
